...
const createQrCodeEntry = (imageUrl = "", destinationUrl = "") => ({
          ...
          {!isEditing && (
            <div className="mt-3 pt-1 pb-2">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                {!isEditMode && qrCodeEntries.some((entry) => entry.destinationUrl) && (
                  <div className="overflow-hidden rounded-xl border border-border/80 bg-background/70 shadow-[inset_0_1px_0_hsl(var(--background)/0.7)]">
                    <div className="transition-transform duration-200 ease-out will-change-transform" style={{ display: 'grid', gridTemplateColumns: '100% 100%', transform: pendingUrlLabel === 'QR Code' ? 'translateX(-100%)' : 'translateX(0)' }}>
                      <button onClick={() => openUrl(firstQrEntryWithDestination?.destinationUrl || "", "QR Code")} className="group flex w-full items-center gap-2 bg-muted/35 px-2.5 py-0.5 transition-colors hover:bg-muted/65 active:opacity-75">
                        <QrCode className="h-5 w-5 text-orange-500 shrink-0 p-1" />
                        <span className="flex-1 text-left text-[11px] font-semibold text-foreground">QR Code{qrCodeEntries.length > 1 ? "s" : ""}</span>
                        <ChevronRight className="size-3 text-muted-foreground/50 group-hover:text-muted-foreground transition-colors shrink-0" />
                      </button>
                      <div className="relative overflow-hidden bg-background/80">
                        ...
                        <div className="flex items-center gap-2 px-3 py-1 pl-5">
                          <button
                            onClick={() => openUrl(firstQrEntryWithDestination?.destinationUrl || "", "QR Code")}
                            className="flex-1 min-w-0 flex items-center gap-2.5 text-left"
                          >
                            <QrCode className="h-5 w-5 text-orange-500 shrink-0 p-1" />
                            <p className="min-w-0 truncate text-[11px] font-semibold text-foreground leading-tight">Open QR Code{qrCodeEntries.length > 1 ? "s" : ""}?</p>
                          </button>
                          <div className="flex items-center shrink-0">
