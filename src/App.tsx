      ...
      setHomeRouteDialogPoints(
        Array.isArray(selected.deliveryPoints)
          ? selected.deliveryPoints.map((point: {
              ...
              documents: Array.isArray(point.documents)
                ? point.documents
                    .map((document) => {
                      ...
                      return {
                        id: document?.id?.trim() || `${Date.now()}-${Math.random().toString(16).slice(2)}`,
                        name,
                        mimeType: document?.mimeType?.trim() || "application/octet-stream",
                        size: Number(document?.size) || 0,
                        url,
                        kind: document?.kind === "pdf" || document?.kind === "zip" || document?.kind === "file"
                          ? document.kind
                          : "file",
                      }
                    })
