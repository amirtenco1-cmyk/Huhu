...
function buildMessageDedupKey(msg) {
  ...
  const participantJid = String(msg?.key?.participant || '').trim();
...
function buildContentDedupKey(remoteJid, senderJid, text, message) {
  const normalizedRemoteJid = String(remoteJid || '').trim();
  const normalizedSenderJid = String(senderJid || '').trim();
  const normalizedText = String(text || '').trim().toLowerCase();
  if (!normalizedRemoteJid || !normalizedSenderJid || !normalizedText) return '';

  const contextInfo = getMessageContextInfo(message);
  const quotedStanzaId = String(contextInfo?.stanzaId || '').trim();
  return `${normalizedRemoteJid}|${normalizedSenderJid}|${normalizedText}|${quotedStanzaId}`;
}

function shouldProcessIncomingContent(remoteJid, senderJid, text, message, now = Date.now()) {
  const dedupKey = buildContentDedupKey(remoteJid, senderJid, text, message);
  if (!dedupKey) return true;
...
async function sendLocationResponse(sock, jid, route, point) {
  ...
  if (pdfLinks.length > 0) {
    await sendLocationLinksWithFallback(
      sock,
      jid,
      pdfLinks,
      `${locationMessage}\n\nDokumen PDF:`,
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      if (shouldReconnect) {
        ...
        startBot(overrides).catch((err) => {
          onStatus?.('error');
          console.error('Restart after logout failed:', err);
        }).finally(() => {
          botReconnectInProgress = false;
        });
      }
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        if (!text) continue;
        if (!shouldProcessIncomingContent(remoteJid, senderJid, text, msg.message)) continue;
